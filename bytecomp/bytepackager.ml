  ...
  try
    ...
    let events, debug_dirs =
      if !Clflags.debug && compunit.cu_debug > 0 then begin
        ...
        let unit_events =
          (Compression.Marshal.from_channel ic : debug_event list) in
        let events =
          rev_append_map
            ...
            state.events in
        let unit_debug_dirs =
          (Compression.Marshal.from_channel ic : string list) in
        let debug_dirs =
          String.Set.union
            state.debug_dirs
...
let package_object_files ~ppf_dump files target coercion =
  ...
  Fun.protect ~finally:(fun () -> close_out oc) (fun () ->
    ...
    if !Clflags.debug && state.events <> [] then begin
      Compression.Marshal.to_channel oc (List.rev state.events) [];
      Compression.Marshal.to_channel oc (String.Set.elements state.debug_dirs)
                                        []
    end;
    let force_link =
