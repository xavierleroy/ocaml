...
type error =
  ...
  | Invalid_identifier of Shortident.error
...
let check_label_name lexbuf name =
  if Shortident.is_uppercase name then error lexbuf (Capitalized_label name);
  ...
  | Invalid_directive (dir, explanation) ->
      Location.errorf ~loc "Invalid lexer directive %S%t" dir
        (fun ppf -> match explanation with
           | None -> ()
           | Some expl -> fprintf ppf ": %s" expl)
  | Invalid_identifier err ->
      Location.error ~loc (Shortident.describe_error err)
...
rule token = parse
  ...
  | identstart_ext identchar_ext * as raw_name
      { let name = ident_for_extended lexbuf raw_name in
        if Shortident.is_uppercase name
        then UIDENT name
        else LIDENT name }
