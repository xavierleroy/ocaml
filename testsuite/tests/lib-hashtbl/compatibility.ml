...
let check_failure (h: (string, int) Hashtbl.t) vnum =
  try
    ignore (Hashtbl.find_opt h ""); assert false
  with Hashtbl.Unsupported_version_number n ->
    assert (n = vnum)

let check_table version h expected =
  assert (Hashtbl.version_number h = version);
  assert (version <= Hashtbl.last_version_number);
  if version >= Hashtbl.first_supported_version_number
  then check_contents h expected
  else check_failure h version;
...
let _ =
  check_table 1 h1 ["one", 1; "two", 2; "three", 3; "four", 4];
  check_table 2 h2 ["cinq", 5; "six", 6; "sept", 7; "huit", 8; "neuf", 9]
