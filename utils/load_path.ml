...
module Dir = struct
  ...
  let find_uncap t fn =
    let fn = Unident.uncapitalize fn in
    let search base =
      if Unident.uncapitalize base = fn then
...
let prepend_add dir =
  List.iter (fun base ->
      ...
      STbl.replace !files_uncap (Unident.uncapitalize base) fn
    ) dir.Dir.files
  ...
  List.iter
    (fun base ->
       ...
       if not (STbl.mem !files base) then
         STbl.replace !files base fn;
       let ubase = Unident.uncapitalize base in
...
let find_uncap fn =
  ...
  try
    if is_basename fn && not !Sys.interactive then
      STbl.find !files_uncap (Unident.uncapitalize fn)
    else
      Misc.find_in_path_uncap (get_paths ()) fn
  with Not_found ->
    let fn_uncap = Unident.uncapitalize fn in
